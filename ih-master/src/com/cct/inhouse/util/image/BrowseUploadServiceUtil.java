package com.cct.inhouse.util.image;
...
public class BrowseUploadServiceUtil {
		...
		try {
			...
			for (int i = 0; i < file.getFileUploadFileNameTmp().length; i++) {
				if (file.getDeleteFlag()[i].trim().equals("Y")) {
					// Clear temp
					FileManagerUtil.deleteQuietly(IHUtil.getParameter().getAttachFile().getDefaultTempPath() + file.getFileUploadFileNameTmp()[i]);
				} else {
					// Rename
					renameStr[i] = renameAttachFile(file.getFileUploadFileNameTmp()[i], idPk, (i + 1));

					// Copy to server
					FileManagerUtil.copy(IHUtil.getParameter().getAttachFile().getDefaultTempPath() + file.getFileUploadFileNameTmp()[i], serverPath + renameStr[i]);
			...
			for (int i = 0; i < lstFile.size(); i++) {
				if (lstFile.get(i).getDeleteFlag() != null && lstFile.get(i).getDeleteFlag().trim().equals("Y")) {
					...
					if (StringUtil.nullToString(lstFile.get(i).getId()).trim().equals("")) {
						// Clear temp
						FileManagerUtil.deleteQuietly(IHUtil.getParameter().getAttachFile().getDefaultTempPath() + lstFile.get(i).getFileUploadFileNameTmp());
					} else {
						// Clear server
					...
					// Copy to server
					FileManagerUtil.copy(IHUtil.getParameter().getAttachFile().getDefaultTempPath() + lstFile.get(i).getFileUploadFileNameTmp(), serverPath + renameStr[i]);
	...
	public static Map<String, String> loadMapContentType() throws Exception {
		...
		try {
			is = BrowseUploadServiceUtil.class.getClassLoader().getResourceAsStream("com/cct/inhouse/util/image/fileContent.properties");
			properties.load(is);
	...
	private static String renameAttachFile(String fileNameTmp, String idPk, int indexFile) throws Exception {
		...
		try {
			...
			renameStr.append(StringDelimeter.UNDERLINE.getValue()).append(idPk);
			renameStr.append(StringDelimeter.UNDERLINE.getValue()).append(indexFile);
			renameStr.append(StringDelimeter.DOT.getValue()).append(filenameTmpSplit[1]);
	...
	public static String createThumbnail(String fileName, String contentType) throws Exception {
		...
		try {
			// Check file type
			thumbFile = IHUtil.getMapContenType().get(contentType.toLowerCase());

			DefaultLogUtil.INITIAL.debug("file.getFileUploadContentType()[0] : " + contentType);
			DefaultLogUtil.INITIAL.debug("map with MapContenType : " + thumbFile);

			if (thumbFile == null) {
				thumbFile = fileName;
			} else {
				thumbFile = IHUtil.getParameter().getAttachFile().getImageDefaultPath() + thumbFile;
			}

			File file2 = new File(thumbFile);
			if (!file2.exists()) {
				thumbFile = IHUtil.getParameter().getAttachFile().getImageDefaultPath() + "no_image.jpg";
			}
	...
	public static String displayImage(String fileName, String contentType) throws Exception {
		...
		try {
			// Check file type
			thumbFile = IHUtil.getMapContenType().get(contentType);
