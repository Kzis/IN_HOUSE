...
public class TimeOffsetHistoryService extends AbstractService {

	private TimeOffsetHistoryDAO dao = null;
	...
	public TimeOffsetHistoryService(Logger logger, CommonUser user, Locale locale) {
		super(logger, user, locale);
		this.dao = new TimeOffsetHistoryDAO(logger, SQLPath.TO_HISTORY.getSqlPath(), user, locale);
	...
	protected long countData(CCTConnection conn, HistorySearchCriteria criteria) throws Exception {
		return dao.countData(conn, criteria);
	}

	protected List<HistorySearch> search(CCTConnection conn, HistorySearchCriteria criteria) throws Exception {
		return dao.search(conn, criteria);
	}

	protected ResultSet searchReport(CCTConnection conn, HistorySearchCriteria criteria, Statement stmt) throws Exception {
		return dao.searchReport(conn, stmt, criteria);
	}

	protected XSSFWorkbook exportReport(HistorySearchCriteria criteria, ResultSet rst) throws Exception {
	...
	private int createHeader(int curRow, HistorySearchCriteria criteria, String sheetName) throws Exception {
